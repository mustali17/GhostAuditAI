    ...
    <nav className="space-y-1">
      {navItems.map((item) => {
        ...
        return (
          ...
          >
            {isActive && <div className="absolute left-0 top-0 bottom-0 w-1 bg-primary rounded-r-full shadow-[0_0_10px_#7c3aed]" />}
    ...
    <div className="flex min-h-screen bg-background text-foreground font-sans selection:bg-primary/20 selection:text-primary">
      ...
      <aside className="hidden w-72 border-r border-border/40 bg-card/30 backdrop-blur-xl lg:flex flex-col p-6 sticky top-0 h-screen z-30">
        <div className="mb-10 flex items-center space-x-3 px-2">
          ...
          <span className="text-xl font-bold tracking-tight bg-gradient-to-r from-foreground to-foreground/60 bg-clip-text text-transparent">GhostAudit</span>
        </div>
        ...
        <div className="mt-auto pt-6 border-t border-border/40">
             ...
             <Button variant="ghost" className="w-full justify-start text-muted-foreground hover:text-red-500 hover:bg-red-500/10 h-9 px-2" onClick={handleLogout}>
        ...
        <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-border/40 bg-background/80 backdrop-blur-md px-4 lg:px-8 transition-all">
            <div className="flex items-center gap-4">
                ...
                <Sheet>
                    ...
                    <SheetContent side="left" className="w-72 p-6 border-r border-border/40 bg-background/95 backdrop-blur-xl z-[100]">
                        <SheetHeader className="mb-8 text-left">
                            <SheetTitle className="flex items-center space-x-3">
                                ...
                                <span className="font-bold text-xl tracking-tight text-foreground">GhostAudit</span>
                            </SheetTitle>
                        </SheetHeader>
                        <NavContent />
                        <div className="absolute bottom-6 left-6 right-6">
                            <Button variant="outline" className="w-full justify-start text-red-500 hover:text-red-500 hover:bg-red-500/10 border-red-500/20" onClick={handleLogout}>
                ...
                <div className="hidden md:flex items-center text-sm text-muted-foreground">
                    ...
                    <span className="font-medium text-foreground">{navItems.find(i => i.href === pathname)?.label || 'Overview'}</span>
                </div>
            ...
            <div className="flex items-center gap-3">
                ...
                <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        <Button variant="ghost" size="icon" className="rounded-full ring-2 ring-border/50 hover:ring-primary/50 transition-all">
                    ...
                    <DropdownMenuContent align="end" className="w-56 border-border/40 bg-card/95 backdrop-blur-xl">
                        ...
                        <DropdownMenuItem onClick={() => router.push('/dashboard/settings')} className="cursor-pointer">Profile</DropdownMenuItem>
                        <DropdownMenuItem onClick={() => router.push('/dashboard/settings/billing')} className="cursor-pointer">Billing</DropdownMenuItem>
                        <DropdownMenuSeparator className="bg-border/40" />
                        <DropdownMenuItem onClick={handleLogout} className="text-red-500 focus:text-red-500 focus:bg-red-500/10 cursor-pointer">Logout</DropdownMenuItem>
                    </DropdownMenuContent>
                </DropdownMenu>
            </div>
        </header>
