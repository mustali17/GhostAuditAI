  ...
  const generateSuggestions = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        'http://localhost:5000/api/audit/humanize',
        { sentence },
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="bg-zinc-950 border-zinc-800 text-white sm:max-w-[525px]">
          ...
          {suggestions.length > 0 && (
           ...
           {!suggestions.length && (
               <div className="flex justify-center py-4">
                   <Button onClick={generateSuggestions} disabled={loading} className="bg-white text-black hover:bg-zinc-200">
        ...
        <DialogFooter>
            {suggestions.length > 0 && (
                <Button onClick={() => onApply(selectedSuggestion)} disabled={!selectedSuggestion} className="bg-green-600 hover:bg-green-700 text-white">
                    Apply Changes
                </Button>
            )}
        </DialogFooter>
