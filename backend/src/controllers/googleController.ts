...
export const syncFiles = async (req: Request, res: Response) => {
  ...
  if (!user || !user.googleAccessToken || !user.watchFolderId) {
    return res.status(400).json({ message: 'Google Drive not connected or Watchfolder not set' });
  }
  ...
  try {
    ...
    for (const file of files) {
        ...
        const existingAudit = await AuditResult.findOne({ fileId: file.id, userId });
    ...
    res.json({ message: `Synced ${syncedCount} new files`, totalFound: files.length });
  } catch (error: any) {
